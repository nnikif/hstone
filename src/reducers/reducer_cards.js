 ...
 */
import {LOAD_CARDS, MANA_FILTER, CHANGE_SHIFT, USE_FILTERS, SET_RACE, SET_DETAIL, SET_RARITY, SET_TITLE} from '../actions';

import _ from 'lodash';
const INIT_STATE = {"data":[], "filtered":[], "shift":0, 'mana':0, 'race':'', 'detail':'', 'rarity': 'Any', 'title':''}
  ...
  switch (action.type) {
    ...
    case USE_FILTERS:
      ...
      var title_ok=false;
        ...
        if (card.name) {title_ok=card.name.toLowerCase().includes(state.title.toLowerCase())}
        if (card.rarity) {rarity_ok= (card.rarity===state.rarity)}
        return (card.cost>=state.mana)&& (race_ok||state.race==='') && (desc_ok ||state.detail==='')
      && (rarity_ok ||state.rarity==='Any') && (title_ok ||state.title==='')}
      )
